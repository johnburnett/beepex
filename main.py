...
def chat_details_to_html(fout, chat_details):
    ...
    fout.write("<dt>participants</dt><dd>\n")
    fout.write("<ul>\n")
...
def message_to_html(ctx: ExportContext, chat_details, msg):
    ...
    sec_class = "you" if ctx.self_id == msg.get("senderID") else "them"
    ctx.fout.write(f'<section class="{sec_class}">{msg["senderName"]}:<br>\n')
    if "text" in msg:
...
def copy_css_files(output_root_dir_path, data_dir_name):
    ...
    css_url_sub_dir = posixpath.join(
        data_dir_name, "css"
    )  # relative to root output dir
...
def dump_data(data, output_root_dir):
    ...
    for chat_id, msgs in chat_to_messages.items():
        ...
        output_dir_path = os.path.join(
            output_root_dir, network_dir_name, output_dir_name
        )
